...
class SetDayCommand < Command
  def process(msg, userid, chatid, orig)
    ...
    if days.nil?
      @messageSender.process(I18n.t('setday.command_inline'), chatid, getDaysButtons(calendar_id))
      return
    end
    days = begin
               Integer(days)
  ...
  def getDaysButtons(calendar_id)
    btns = (0..14).map { |n| Telegram::Bot::Types::InlineKeyboardButton.new(text: n.to_s, callback_data: "/setday #{calendar_id} #{n}") }
